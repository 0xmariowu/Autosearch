...
async function main() {
  try {
    ...
    console.log("Downloading AutoSearch installer...");
    const script = await download(INSTALL_URL);

    const tmp = path.join(os.tmpdir(), `autosearch-install-${Date.now()}.sh`);
    fs.writeFileSync(tmp, script, { mode: 0o755 });

    console.log("Installing AutoSearch for Claude Code...\n");
