...
ls ~/Dev/AIMD/ai-recommendations/$(date +%Y-%m-%d).md

# 最近的 scout 日志
tail -30 ~/.local/log/armory-scout/scout.log

# pattern 积累趋势
wc -l ~/Dev/autosearch/patterns.jsonl

# outcome 状态
python3 ~/Dev/autosearch/outcomes.py         # record new intakes
python3 ~/Dev/autosearch/outcomes.py track   # update WHEN/USE counts
```
...
```bash
# Dry-run（不实际搜索，只看配置）
python3 ~/Dev/autosearch/daily.py --dry-run

# 只跑引擎，不跑下游管道
python3 ~/Dev/autosearch/pipeline.py --engine-only

# 完整管道但跳过邮件
python3 ~/Dev/autosearch/pipeline.py --skip-email

# 手动搜索（AI 填参数）
python3 ~/Dev/autosearch/cli.py --config task.json
```
