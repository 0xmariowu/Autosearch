...
async def main(argv: list[str] | None = None) -> int:
    ...
    print()
    print("=" * 62)
    print("  AutoSearch E2B Comprehensive Test Results")
    print("=" * 62)
    print(f"  Overall score:   {summary['overall_score']}/100")
    print(f"  v1.0 readiness:  {emoji} {summary['readiness']}")
    print(f"  Scenarios passed: {summary['passed']}/{summary['total']} ({summary['pass_rate']}%)")
    print(f"  Evidence total:   {summary['total_evidence']} items")
    print(f"  Report chars:     {summary['total_report_chars']}")
    ...
    if summary["failures"]:
        ...
        for f in summary["failures"][:5]:
            print(f"    ❌ {f['id']} {f['name']} (score {f['score']}): {f['error'][:60]}")
    print()
