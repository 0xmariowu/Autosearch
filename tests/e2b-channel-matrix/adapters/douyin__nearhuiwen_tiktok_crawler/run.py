...
REPO = "https://github.com/NearHuiwen/TiktokCrawler"
PLATFORM = "douyin"
PATH_ID = "douyin__nearhuiwen_tiktok_crawler"
WORKSPACE_REPO = Path("/tmp/as-matrix/TiktokCrawler")
...
def _load_upstream_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location("nearhuiwen_tiktok_crawler", MODULE_PATH)
...
def run(query: str, query_category: str) -> dict[str, object]:
    ...
    try:
        ...
        if not hasattr(module, "CrawlerScheduler"):
            raise RuntimeError(
                "sandbox_infeasible: upstream repo does not expose its crawler scheduler"
            )

        raise RuntimeError(
            "sandbox_infeasible: upstream repo only supports share-url/user/challenge/music download flows and does not expose a keyword-search entrypoint for free-text queries"
        )
