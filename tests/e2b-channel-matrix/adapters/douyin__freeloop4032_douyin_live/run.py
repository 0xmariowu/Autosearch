...
REPO = "https://github.com/freeloop4032/douyin-live"
PLATFORM = "douyin"
PATH_ID = "douyin__freeloop4032_douyin_live"
WORKSPACE_REPO = Path("/tmp/as-matrix/douyin-live")
MODULE_PATH = WORKSPACE_REPO / "dy.py"
...
def _load_upstream_module() -> object:
    ...
    spec = importlib.util.spec_from_file_location("freeloop4032_douyin_live", MODULE_PATH)
...
def run(query: str, query_category: str) -> dict[str, object]:
    ...
    try:
        ...
        if not hasattr(module, "parseLiveRoomUrl"):
            raise RuntimeError(
                "sandbox_infeasible: upstream live-room parser entrypoint is missing"
            )
