...
REPO = "https://github.com/kaixindelele/CSDN_pageviews_spider_tomysql_and_visualize"
PLATFORM = "csdn"
PATH_ID = "csdn__kaixindelele_pageviews"
WORKSPACE_REPO = Path("/tmp/as-matrix/CSDN_pageviews_spider_tomysql_and_visualize")
...
def _extract_item_text(item: object) -> str:
    ...
    if not isinstance(item, dict):
        return _clean_text(item)
    for key in ("title", "name", "summary", "description", "content", "text", "blog_id"):
        value = item.get(key)
        if value:
...
def _collect_items(value: object) -> list[object]:
    ...
    if not isinstance(value, dict):
        return []
    items: list[object] = []
    for key in ("data", "items", "result", "results", "list", "blogs"):
...
def _module_candidates() -> list[str]:
    candidates = ["main", "spider", "crawler", "pageviews", "pageview"]
...
def _find_target() -> tuple[Any | None, str | None]:
    attr_names = ("search", "query", "fetch", "crawl", "spider", "get_pageviews")
    for module_name in _module_candidates():
        ...
        for name in dir(module):
            ...
            if not inspect.isclass(candidate):
                continue
            if not any(token in name.lower() for token in ("spider", "crawler", "pageview")):
                continue
            try:
                instance = candidate()
            except Exception:
                continue
            for method_name in ("search", "query", "fetch", "crawl", "get_pageviews"):
...
def _invoke(target: Any, query: str) -> object:
    ...
    for args, kwargs in attempts:
        ...
        try:
            ...
            continue
    raise TypeError("sandbox_infeasible: no compatible upstream invocation")
...
def run(query: str, query_category: str) -> dict[str, object]:
    ...
    try:
        ...
        target, target_name = _find_target()
        if target is None:
            raise RuntimeError(
                "sandbox_infeasible: pageviews spider exposes blog/pageview tracking but no keyword-search entrypoint"
            )
