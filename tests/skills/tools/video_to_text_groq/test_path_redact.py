...
def _load_video_to_text_groq() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location(
        "video_to_text_groq_path_redact_under_test", transcribe_path
    )
...
def test_local_path_not_in_structured_output(
    ...
    with _client(handler) as client:
        success = VIDEO_TO_TEXT_GROQ.transcribe(absolute_input, http_client=client)

    monkeypatch.delenv("GROQ_API_KEY", raising=False)
    failure = VIDEO_TO_TEXT_GROQ.transcribe(absolute_input)
