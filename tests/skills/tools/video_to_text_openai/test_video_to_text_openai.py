...
def _load_video_to_text_openai() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location(
        "video_to_text_openai_under_test", transcribe_path
    )
...
def test_url_happy_path_returns_text_srt_metadata(
    ...
    monkeypatch.setattr(VIDEO_TO_TEXT_OPENAI, "_extract_audio", lambda source: str(fake_mp3))

    def handler(request: httpx.Request) -> httpx.Response:
        assert request.method == "POST"
        assert str(request.url) == VIDEO_TO_TEXT_OPENAI.OPENAI_TRANSCRIPTIONS_URL
    ...
    with _client(handler) as client:
        result = VIDEO_TO_TEXT_OPENAI.transcribe(
...
def test_local_audio_happy_path_skips_yt_dlp(
    ...
    with _client(handler) as client:
        result = VIDEO_TO_TEXT_OPENAI.transcribe(str(fake_mp3), http_client=client)
...
def test_missing_openai_api_key_returns_structured_error(
    ...
    result = VIDEO_TO_TEXT_OPENAI.transcribe(str(fake_mp3))

    assert result["ok"] is False
    assert result["reason"] == "missing_api_key"
    assert (
        result["suggest"]
        == "配置 OPENAI_API_KEY env var（付费 $0.006/min，https://platform.openai.com/api-keys）"
    )
...
def test_openai_429_returns_rate_limited(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert result["suggest"] == "降级 video-to-text-local 或等待"
...
def test_yt_dlp_failure_returns_structured_error(
    ...
    result = VIDEO_TO_TEXT_OPENAI.transcribe("https://unsupported.example/video")
