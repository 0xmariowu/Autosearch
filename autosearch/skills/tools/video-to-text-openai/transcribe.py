...
VideoToTextOpenAIResult = dict[str, object]
...
def transcribe(
    ...
    *,
...
) -> VideoToTextOpenAIResult:
    """Transcribe a video/audio URL or local file through OpenAI Whisper."""
    api_key = os.environ.get("OPENAI_API_KEY")
    if not api_key:
        return _failure(
            source=url_or_path,
            reason="missing_api_key",
            suggest="配置 OPENAI_API_KEY env var（付费 $0.006/min，https://platform.openai.com/api-keys）",
        )
    ...
    try:
        ...
        LOGGER.warning("video_to_text_openai_ffmpeg_missing", source=redact_path_for_output(url_or_path))
    ...
    if response.status_code == 429:
        return _failure(
            source=url_or_path,
            reason="rate_limited",
            suggest="降级 video-to-text-local 或等待",
        )
...
def _extract_audio(url_or_path: str) -> str:
    tmp_dir = Path(tempfile.mkdtemp(prefix="autosearch-video-to-text-openai-"))
